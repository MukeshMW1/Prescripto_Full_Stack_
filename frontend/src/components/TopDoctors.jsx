      ...
      <div className="w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 sm:px-0">
{
    doctors.slice(0,10).map((doctor,index)=>(
        <div onClick={()=>navigate(`/appointment/${doctor._id}`)} className="border border-blue-200 rounded-xl transition-all overflow-hidden cursor-pointer duration-300 hover:-translate-y-3 transform" key={index}>
...
<p className='text-gray-900 text-sm px-2 pb-2 '>{doctor.speciality}</p>
      ...
      <button onClick={()=>{navigate('/doctors');scrollTo(0,0)}} className='border rounded-[20px] px-6 py-1 bg-blue-50'>More</button>
    </div>
  )
}
